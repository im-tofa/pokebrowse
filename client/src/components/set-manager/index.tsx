...
const SetManager: FunctionalComponent = () => {
    ...
    const deleteSets = async () => {
        ...
        if (!response.ok) { setErrorMessage(await response.json()); return; }
    ...
    return (
        <div class={style.profile}>
            <div class={style.top}><h2>My Sets</h2> {selected.length > 0 && <button class={style.btn} onClick={async (e) => {
                e.preventDefault(); 
            ...
            <Results results={results} fetchMore={fetchMore} editable={true} selected={selected} setSelected={setSelected}/>
        </div>
    );
};
